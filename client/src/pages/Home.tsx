        ...
        <div className="container mx-auto px-4 md:px-6 relative z-20">
          ...
          >
            <h1 className="text-5xl md:text-7xl lg:text-8xl font-black tracking-tighter leading-[1.1] text-foreground mb-6">
              {lang === 'ar' ? (
                <>شراريب <span className="text-primary/70">تريحك</span><br/>وتكمل ستايلك.</>
              ) : (
                <>Socks that feel <span className="text-primary/70">good</span><br/>and look better.</>
              )}
            </h1>
      ...
      <section className="py-24 bg-zinc-50 dark:bg-zinc-900/50">
        <div className="container mx-auto px-4 md:px-6">
          <div className="text-center max-w-2xl mx-auto mb-16">
            <h2 className="text-3xl md:text-4xl font-black mb-6">{t('section.whyUs')}</h2>
            <p className="text-muted-foreground text-lg">
              {lang === 'ar' ? 'نهتم بكل تفصيلة لنقدم لك تجربة ارتداء لا مثيل لها.' : 'We care about every detail to provide you with an unmatched wearing experience.'}
            </p>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            <div className="bg-background p-8 rounded-3xl shadow-sm border border-border/50 text-center hover:-translate-y-2 transition-transform duration-300">
              ...
              <h3 className="text-xl font-bold mb-3">{lang === 'ar' ? 'راحة فائقة' : 'Premium Comfort'}</h3>
              <p className="text-muted-foreground">{lang === 'ar' ? 'قطن طبيعي يسمح بالتهوية.' : 'Natural cotton that allows breathability.'}</p>
            </div>
            <div className="bg-background p-8 rounded-3xl shadow-sm border border-border/50 text-center hover:-translate-y-2 transition-transform duration-300">
              <div className="w-16 h-16 mx-auto bg-primary/10 text-primary rounded-2xl flex items-center justify-center mb-6">
                <ShieldCheck className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-bold mb-3">{lang === 'ar' ? 'جودة تدوم' : 'Durable Quality'}</h3>
              <p className="text-muted-foreground">{lang === 'ar' ? 'خياطة متينة تتحمل الغسيل المتكرر.' : 'Strong stitching that withstands frequent washing.'}</p>
            </div>
            <div className="bg-background p-8 rounded-3xl shadow-sm border border-border/50 text-center hover:-translate-y-2 transition-transform duration-300">
              <div className="w-16 h-16 mx-auto bg-primary/10 text-primary rounded-2xl flex items-center justify-center mb-6">
                <Zap className="w-8 h-8" />
              </div>
              <h3 className="text-xl font-bold mb-3">{lang === 'ar' ? 'تصاميم عصرية' : 'Modern Designs'}</h3>
              <p className="text-muted-foreground">{lang === 'ar' ? 'تناسب العمل، الرياضة، والمنزل.' : 'Perfect for work, sports, and home.'}</p>
            </div>
          </div>
        </div>
      </section>
